...
## Responsibility
Embed a batch of `DocumentChunk` objects into float vectors using the Anthropic
`voyage-3` model via Spring AI's `EmbeddingModel` bean.
...
## Rules
- **Never** call the Anthropic HTTP API directly — always use the injected `EmbeddingModel` bean.
